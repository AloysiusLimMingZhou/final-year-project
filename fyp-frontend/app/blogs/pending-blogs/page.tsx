...
export default function GetPendingBlogs() {
    ...
    return (
        <div>
            ...
            {pendingPosts.map((pendingPost: any) => (
                <div key={pendingPost.id}>
                    <h1>Title: {pendingPost.title}</h1>
                    <p>By: {pendingPost.author_name}</p>
                    ...
                    <button onClick={() => router.push(`/blogs/pending-blogs/${pendingPost.id}`)}>Read More</button>
                </div>
            ))}
            {page > 1 &&
                <button onClick={handlePreviousPagination} className="bg-blue-500 text-white px-4 py-2 rounded">Previous Page</button>
            }
            {pendingPosts.length === limit &&
                <button onClick={handleNextPagination} className="bg-blue-500 text-white px-4 py-2 rounded">Next Page</button>
            }
            {pendingPosts.length === 0 && <p>No posts found!</p>}
            <button onClick={() => router.push('/blogs')} className="bg-blue-500 text-white px-4 py-2 rounded">Return to Blogs</button>
